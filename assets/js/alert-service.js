...
function AlertService() {
    function ConfigFactory(variation, subject, options) {
        switch (variation) {
            case 'validation':
                return {
                    class: 'error',
    ...
    function RenderAlerts(alertsContainer, configs) {
  ...
  `;
        ...
        if (config.dismissCallback) {
            element.querySelector('.icon.icon-close').onclick = config.dismissCallback.bind(element);
        }
        return element;
    }
    function _InjectIntoDOM(template) {
